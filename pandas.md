...
### pd.cut(要切割的数据，bins=用于切割数据的区间边界)
...
grouby（）方法经常跟聚合函数（agg(),count(),max(),min(),sun(),avg())一起使用通过一个或多个列对结果集进行分组。。
...
isna() 方法返回一个 DataFrame 对象，其中所有NA（非数字）值都被布尔值 True 替换 ，否则False。
